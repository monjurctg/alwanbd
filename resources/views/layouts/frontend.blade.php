...
<head>
	...
	<style type="text/css">
		:root {
			...
			;
			--color-light-green:
				{{ $gtext['light_green_color'] }}
			;
			--color-lightness-green:
				{{ $gtext['lightness_green_color'] }}
			;
			--color-gray:
				{{ $gtext['gray_color'] }}
			;
			--color-gray-dark:
				{{ $gtext['dark_gray_color'] }}
			;
			--color-gray-400:
				{{ $gtext['light_gray_color'] }}
			;
			--color-black:
				{{ $gtext['black_color'] }}
			;
...
<body {!! $gtext['is_rtl'] == 1 ? 'class="rtl"' : '' !!}>
	@if($gtext['gtm_publish'] == 1)
		...
		<noscript><iframe src="https://www.googletagmanager.com/ns.html?id={{ $gtext['google_tag_manager_id'] }}" height="0"
